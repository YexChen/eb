    ...
    <div class="publish-body">
      <div class="publish-write">
        <div class="publish-write-block">
          ...
          <div class="publish-write-edit publish-write-single" contenteditable="plaintext-only" @keyup= "changeData('title',$event)">
          </div>
        </div>
        <div class="publish-write-block">
          <span class= "publish-write-title ">摘要</span>
          <div class="publish-write-edit publish-write-single" contenteditable="plaintext-only" @keyup= "changeData('sum',$event)">
          </div>
        </div>
        <div class="publish-write-block">
          <span class= "publish-write-title ">正文</span>
          <div class="publish-write-edit publish-write-multiple" contenteditable="plaintext-only" @keyup= "changeData('main',$event)">
          </div>
        </div>
...
export default {
  ...
  methods:{
    ...
    postForm(){
      ...
      form.append('sum',this.form.sum)
      form.append('content',this.form.main)
      form.append('authorname',this.user.nickName)
      form.append('author',this.user.id)
      form.append('authoravatar',this.user.userAvatar)
      form.append('avatar',this.$refs.image.files[0])
      // for (var key of form.entries()) {
      //   console.log(key[0] + ', ' + key[1])
      // }
      this.$http.post("/blogapi/article/add",form).then(res=>{
        console.log(res)
        if(res.data.code== '0'){
          alert(res.data.msg)
          this.$router.push("/")
